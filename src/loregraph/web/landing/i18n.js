...
window.LG_I18N = {
  ...
  "lib.shelf3d.hint":  { "en": "Click a book to take it off the shelf.", "zh-CN": "点一本书，把它取下来。", "zh-TW": "點一本書，把它取下來。", "ja": "本をクリックすると、棚から取り出せます。", "ko": "책을 클릭하면 서가에서 꺼낼 수 있습니다.", "fr": "Cliquez sur un livre pour le sortir du rayon.", "es": "Haz clic en un libro para sacarlo del estante.", "de": "Klicke ein Buch an, um es aus dem Regal zu nehmen." },
  ...
  // A work on the shelf that has no graph — either because its file is still on
  // its way, or because the pipeline has never been run over it. Most of this
  // library is the second case.
  "empty.bookLoading":    { "en": "Loading this work's graph…", "zh-CN": "正在载入这部作品的图谱……", "zh-TW": "正在載入這部作品的圖譜……", "ja": "この作品のグラフを読み込んでいます…", "ko": "이 작품의 그래프를 불러오는 중…", "fr": "Chargement du graphe de cette œuvre…", "es": "Cargando el grafo de esta obra…", "de": "Graph dieses Werks wird geladen…" },
  "empty.bookNone.title": { "en": "Nothing extracted from this work yet", "zh-CN": "这部作品还没有抽取结果", "zh-TW": "這部作品還沒有抽取結果", "ja": "この作品はまだ抽出されていません", "ko": "이 작품은 아직 추출되지 않았습니다", "fr": "Rien n'a encore été extrait de cette œuvre", "es": "Todavía no se ha extraído nada de esta obra", "de": "Aus diesem Werk wurde noch nichts extrahiert" },
  "empty.bookNone.body":  { "en": "It is on the shelf, but the pipeline has not been run over it — so there are no passages, entities or relations to show here. Two works in this library have been through it; open one of those to see what this view does.", "zh-CN": "它在书架上，但还没跑过流水线，所以这里没有段落、实体和关系可看。书库里有两部作品跑完了，可以先打开那两部看看这个视图能做什么。", "zh-TW": "它在書架上，但還沒跑過流水線，所以這裡沒有段落、實體和關係可看。書庫裡有兩部作品跑完了，可以先打開那兩部看看這個視圖能做什麼。", "ja": "棚には並んでいますが、パイプラインをまだ通していないため、ここに表示できる原文・エンティティ・関係がありません。処理済みの作品が二点あるので、まずはそちらでこのビューの働きをご覧ください。", "ko": "서가에는 있지만 파이프라인을 아직 돌리지 않아 여기에 보여줄 구절과 엔티티, 관계가 없습니다. 처리를 마친 작품이 두 편 있으니 먼저 그것들로 이 화면이 무엇을 하는지 보십시오.", "fr": "Elle est sur l'étagère, mais le pipeline n'a pas encore tourné dessus : ni passages, ni entités, ni relations à montrer ici. Deux œuvres de cette bibliothèque y sont passées ; ouvrez l'une d'elles pour voir ce que fait cette vue.", "es": "Está en la estantería, pero el pipeline aún no se ha ejecutado sobre ella: no hay pasajes, entidades ni relaciones que mostrar aquí. Dos obras de esta biblioteca sí han pasado por él; abre una de ellas para ver qué hace esta vista.", "de": "Es steht im Regal, aber die Pipeline ist noch nicht darüber gelaufen — es gibt hier also keine Textstellen, Entitäten oder Beziehungen zu zeigen. Zwei Werke dieser Bibliothek haben sie durchlaufen; öffnen Sie eines davon, um zu sehen, was diese Ansicht kann." },
